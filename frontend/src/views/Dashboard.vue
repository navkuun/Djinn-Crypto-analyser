...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      const data = {
        crypto_name: this.selectedCryptocurrency,
        time_range: this.selectedTime,
      };

      await axios
        .post("http://localhost:5000/crypto-input", data, {
          withCredentials: false,
        })
        .then((res) => {
          console.log(res);
          localStorage.setItem("crypto_data", res);
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  computed: {},
  created() {
    ipcRenderer.send("resize-window-main", 1100, 600, true, "Dashboard", true);
  },
  // async mounted() {
  //   await axios
